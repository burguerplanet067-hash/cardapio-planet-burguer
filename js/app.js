...
(function () {
  ...
  // Valor numérico da entrega a somar no total (0 quando o modo é
  // "consultar", já que o valor ainda não é conhecido nesse caso).
  function getDeliveryFeeAmount() {
    return DELIVERY_FEE_MODE === "fixo" ? DELIVERY_FEE : 0;
  }

  // Texto curto para representar a taxa de entrega (usado ao lado do
  // rótulo "Taxa de entrega" ou "Entrega" nas telas do site).
  function getDeliveryFeeDisplayText() {
    return DELIVERY_FEE_MODE === "fixo" ? formatBRL(DELIVERY_FEE) : DELIVERY_FEE_TEXT;
  }
  ...
  function renderStoreInfo() {
    ...
    const fbName = STORE_INFO.facebook || "";
    const fbUrl = "https://facebook.com/" + fbName.replace(/\s+/g, "");
    document.getElementById("heroFacebook").href = fbUrl;
    document.getElementById("heroFacebookLabel").textContent = fbName || "Facebook";
    document.getElementById("footerFacebook").href = fbUrl;
    document.getElementById("footerFacebookLabel").textContent = fbName || "Facebook";
    ...
    document.getElementById("infoDelivery").textContent =
      `Entrega: ${getDeliveryFeeDisplayText()} · pedido mín. ${formatBRL(MINIMUM_ORDER)}`;
  ...
  function updateCheckoutSummary() {
    ...
    const deliveryFee = currentOrderType === "entrega" ? getDeliveryFeeAmount() : 0;
    ...
    if (currentOrderType === "entrega") {
      document.getElementById("checkoutDeliveryFee").textContent = getDeliveryFeeDisplayText();
  ...
  function renderConfirmation(data) {
    ...
    const subtotal = cartTotal();
    const deliveryFee = data.tipo === "entrega" ? getDeliveryFeeAmount() : 0;
    const total = subtotal + deliveryFee;
    ...
    if (data.tipo === "entrega") {
      document.getElementById("confirmDeliveryFee").textContent = getDeliveryFeeDisplayText();
      deliveryRow.classList.remove("hidden");
    } else {
      deliveryRow.classList.add("hidden");
    }
  ...
  function buildWhatsappMessage(data) {
    ...
    if (data.tipo === "entrega") {
      linhas.push(`Taxa de entrega: ${getDeliveryFeeDisplayText()}`);
    }
    linhas.push(`*TOTAL: ${formatBRL(total)}*`);
    if (data.tipo === "entrega" && DELIVERY_FEE_MODE === "consultar") {
      linhas.push("(valor da entrega será combinado aqui pelo WhatsApp)");
    }
